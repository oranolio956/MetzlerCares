...
serve(async (req) => {
  ...
  try {
    ...
    if (appError || !application) {
      ...
      return new Response(
        JSON.stringify({ error: 'Application not found or not approved for disbursement' }),
    ...
    if (!partner.stripe_account_id || !partner.banking_verified) {
      ...
      return new Response(
        JSON.stringify({ error: 'Partner banking information not verified' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    try {
      ...
      console.log(JSON.stringify({ level: 'info', ctx: 'disburse', msg: 'transfer_created', transfer_id: transfer.id }));
      ...
      try {
        ...
        if (checkpointError) {
          console.error(JSON.stringify({ level: 'error', ctx: 'disburse', msg: 'checkpoint_failed', error: checkpointError }));
          // Don't fail the payment for this - log and continue
        } else {
          console.log(JSON.stringify({ level: 'info', ctx: 'disburse', msg: 'checkpoint_created', data: checkpointData }));
        }
      } catch (checkpointErr) {
      ...
      console.log(JSON.stringify({ level: 'info', ctx: 'disburse', msg: 'disbursed', amount: transferAmount, facility_name: partner.facility_name, application_id }));
      ...
      console.error(JSON.stringify({ level: 'error', ctx: 'disburse', msg: 'transfer_failed', error: stripeError }));
    ...
    console.error(JSON.stringify({ level: 'error', ctx: 'disburse', msg: 'unexpected_error', error }));
    ...
    try {
      ...
      console.error(JSON.stringify({ level: 'error', ctx: 'disburse', msg: 'keragon_escalation_failed', error: keragonError }));
    }
...
async function updatePublicMetrics(supabase: any) {
  try {
    ...
    if (countError) {
      console.error(JSON.stringify({ level: 'error', ctx: 'metrics', msg: 'count_failed', error: countError }));
      return;
    }
    ...
    if (sumError) {
      console.error(JSON.stringify({ level: 'error', ctx: 'metrics', msg: 'sum_failed', error: sumError }));
      return;
    }
    ...
    if (updateError) {
      console.error(JSON.stringify({ level: 'error', ctx: 'metrics', msg: 'update_failed', error: updateError }));
    } else {
      console.log(JSON.stringify({ level: 'info', ctx: 'metrics', msg: 'updated', beneficiaries: totalBeneficiaries || 0, total_funds: totalFunds }));
    }

  } catch (error) {
    console.error(JSON.stringify({ level: 'error', ctx: 'metrics', msg: 'unexpected_error', error }));
  }
}
