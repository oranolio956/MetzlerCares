...
serve(async req => {
  ...
  try {
    ...
    if (!application_id || !beneficiary_id) {
      return new Response(JSON.stringify({ error: 'application_id and beneficiary_id are required' }), {
        status: 400,
    ...
    const checkpoints = [{ interval_days: 30 }, { interval_days: 60 }, { interval_days: 90 }]
