<script lang="ts">
  ...
  export let hashtags: string[] = ['RecoveryHousing', 'SoberLiving']
  export let platforms: string[] = ['facebook', 'twitter', 'linkedin', 'email']
  export let size: 'sm' | 'md' | 'lg' = 'md'
  export let variant: 'horizontal' | 'vertical' = 'horizontal'
  ...
  function getShareUrl(platform: string): string {
    ...
    switch (platform) {
      ...
      case 'email':
        const subject = encodeURIComponent(`${title} - Metzler Foundations`)
  ...
  function copyToClipboard() {
    navigator.clipboard.writeText(currentUrl).then(() => {
      trackEvent('link_copied', { page_url: currentUrl })
