...
<div class="fixed top-4 right-4 z-[100] space-y-3" role="region" aria-label="Notifications">
  {#each $items as t (t.id)}
    <div class={`text-cream rounded-md shadow-md px-4 py-3 flex items-start gap-3 ${bg(t.type)}`} role="status" aria-live="polite">
      <span class="sr-only">{sr(t.type)}:</span>
      <div class="flex-1">{t.message}</div>
      <button class="text-cream text-opacity-80 hover:text-cream" aria-label="Dismiss" on:click={() => dismiss(t.id)}>✕</button>
    </div>
  {/each}
</div>