...
export class ColoradoSEOWorker {
  ...
  private async generateCityPage(task: ContentGenerationTask): Promise<any> {
    ...
    // Find location data
    const locationData = COLORADO_LOCATIONS.find(
      loc => loc.city.toLowerCase().replace(/\s+/g, '-') === location.toLowerCase()
    )
    ...
    const contentBrief = await keywordResearcher.generateContentBrief(`${locationData.city} sober living`, keywords)
