...
export class SEOPageGenerator {
  ...
  private generateCityServicePage(
    city: ColoradoLocation,
    service: ServiceType,
  ...
  private generateMetaDescription(city: string, service: ServiceType, cluster: KeywordCluster): string {
    ...
    return `${baseDescriptions[service as keyof typeof baseDescriptions]} ${trustSignal}. ${
      competitorGap ? `Better alternative to ${competitorGap}.` : ''
    }`
  }
  ...
  private generateCompetitorBeatingContent(
    city: ColoradoLocation,
    service: ServiceType,
    cluster: KeywordCluster
  ): string {
    const competitorWeaknesses = {
      'ripoff report':
        'Unlike facilities featured in Ripoff Report complaints, our program is fully licensed, transparent about costs, and provides evidence-based care with proven outcomes.',
      ...
      complaints:
        'While other facilities have numerous complaints, we maintain a 95% client satisfaction rate and are accredited by national healthcare organizations.',
      lawsuit:
        'Unlike facilities involved in lawsuits, we operate with full legal compliance and maintain the highest ethical standards in addiction treatment.'
    }
  ...
  private generateImageList(city: string, service: ServiceType): Array<{ alt: string; prompt: string }> {
    const prompts = {
      detox: [
        {
          alt: 'Medical detox facility in Colorado',
          prompt:
            'Modern medical detox facility building in Colorado, professional healthcare setting, welcoming entrance'
        },
        {
          alt: 'Comfortable detox room',
          prompt: 'Comfortable private room for medical detox, medical equipment, peaceful environment'
        },
        {
          alt: 'Medical staff supervision',
          prompt: 'Medical professionals monitoring patient during detox, 24/7 care, professional healthcare team'
        }
      ],
      rehab: [
        {
          alt: 'Addiction treatment center',
          prompt: 'Professional addiction treatment center in Colorado, therapy rooms, group spaces'
        },
        {
          alt: 'Group therapy session',
          prompt: 'Group therapy session for addiction recovery, supportive environment, professional facilitation'
        },
        {
          alt: 'Individual counseling room',
          prompt: 'Private counseling room for addiction therapy, comfortable setting, therapeutic environment'
        }
      ],
      'sober-living': [
        {
          alt: 'Sober living home',
          prompt: 'Comfortable sober living home in Colorado, residential setting, recovery supportive environment'
        },
        {
          alt: 'Shared living space',
          prompt: 'Shared living space in sober home, clean environment, recovery-focused setting'
        },
        {
          alt: 'Community kitchen',
          prompt: 'Shared kitchen in sober living facility, community space, recovery housing amenities'
        }
      ],
      aftercare: [
        {
          alt: 'Aftercare support meeting',
          prompt: 'Professional aftercare support group meeting, recovery community, ongoing support'
        },
        {
          alt: 'Recovery coaching session',
          prompt: 'Individual recovery coaching session, professional guidance, long-term support'
        },
        {
          alt: 'Alumni program event',
          prompt: 'Treatment center alumni program gathering, recovery celebration, community support'
        }
      ],
      'competitor-gap': [
        {
          alt: 'Licensed treatment facility',
          prompt:
            'Professional licensed addiction treatment facility, accreditation certificates, legitimate healthcare setting'
        },
        {
          alt: 'Treatment center verification',
          prompt: 'Treatment center license verification process, professional accreditation, quality assurance'
        },
        {
          alt: 'Recovery professional consultation',
          prompt: 'Professional addiction treatment consultation, expert guidance, recovery planning'
        }
      ]
    }
  ...
  private generateInformationalPages(): GeneratedPage[] {
    ...
    informationalTopics.forEach(topic => {
      pages.push({
        ...
        images: [
          {
            alt: 'Colorado recovery community',
            prompt: 'Colorado recovery community support, group meetings, healing environment'
          }
        ]
      })
    })
