...
export class ContentVariation {
  ...
  static generateSentence(
    type: 'intro' | 'benefit' | 'community',
    context: { city: string; county?: string; service?: string; benefit?: string },
    seed: number
  ): string {
  ...
  static enhanceWithContext(
    content: string,
    context: { population?: number; demographics?: string; facilities?: number }
  ): string {
