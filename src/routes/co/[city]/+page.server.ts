...
export const load: PageServerLoad = async ({ params, setHeaders, url }) => {
  ...
  try {
    ...
    const location = locations.find(loc => loc.city.toLowerCase().replace(/\s+/g, '-') === city.toLowerCase())
    ...
    coloradoIndexingAPI
      .submitUrls([pageUrl], 'URL_UPDATED')
      .then((response: any) => {
    ...
    const nearbyServices = COLORADO_RECOVERY_SERVICES.filter(
      svc => calculateDistance(location.coordinates, svc.coordinates) <= 50
    )
      .map(svc => ({
        type: svc.type,
        name: svc.name,
