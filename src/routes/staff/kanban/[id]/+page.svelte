  ...
  <header class="bg-white shadow-sm border-b">
    <div class="container mx-auto px-6 py-4">
      <div class="flex items-center justify-between">
        ...
        <button on:click={() => goto('/staff/dashboard')} class="btn btn-outline"> Back to Dashboard </button>
      </div>
    </div>
  </header>
        ...
        <button
          on:click={async () => {
            ...
            if (!boardError && boardData) {
              ...
              const { data: columnsData } = await supabase.from('kanban_columns').insert(defaultColumns).select()
              ...
              <div class="space-y-3">
                {#each column.cards as card}
                  <div
                    ...
                    class="bg-white rounded-lg p-4 shadow-sm border-l-4 {categoryColors[
                      card.category
                    ]} cursor-move hover:shadow-md transition-all duration-200 group kanban-card"
                    role="button"
      ...
      <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4">
        ...
        <div class="space-y-6">
          <div>
            ...
            <div id="time-minutes" class="grid grid-cols-3 gap-3">
              <button
                ...
                class="px-4 py-2 rounded-lg border-2 transition-colors {timeMinutes === 15
                  ? 'border-primary bg-primary text-white'
                  : 'border-gray-200 text-gray-dark hover:border-primary'}"
              >
                15 min
              </button>
              <button
                on:click={() => (timeMinutes = 30)}
                class="px-4 py-2 rounded-lg border-2 transition-colors {timeMinutes === 30
                  ? 'border-primary bg-primary text-white'
                  : 'border-gray-200 text-gray-dark hover:border-primary'}"
              >
                30 min
              ...
              <button
                ...
                class="px-4 py-2 rounded-lg border-2 transition-colors {timeMinutes === 45
                  ? 'border-primary bg-primary text-white'
                  : 'border-gray-200 text-gray-dark hover:border-primary'}"
              >
                45 min
          ...
          <div class="flex gap-3">
            ...
            <button on:click={logTime} class="flex-1 px-4 py-2 bg-primary text-white rounded-lg hover:bg-blue-800">
              Log Time
            </button>
          </div>
        </div>
