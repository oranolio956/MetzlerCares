<script lang="ts">
  ...
  import type { PageData, TableOfContentsItem } from '$lib/types';

  export let data;

  let pageData: PageData | null = data.page || null;
  ...
  $: relatedLinks = (() => {
    ...
    if (pageData && pageData._type === 'pillarPage' && Array.isArray(pageData.relatedClusterPages)) {
      for (const p of pageData.relatedClusterPages) {
        if (p?.slug?.current) links.push({ title: p.title, href: `/resources/${p.slug.current}` })
      }
    }
    if (pageData && pageData._type === 'clusterPage' && pageData.pillarPage?.slug?.current) {
      links.push({ title: pageData.pillarPage.title, href: `/resources/${pageData.pillarPage.slug.current}` })
    }
    return links
  })()
...
<svelte:head>
  ...
  <meta name="description" content={pageData?.metaDescription || pageData?.excerpt || 'Resources for sober living and recovery support.'} />
...
<div class="min-h-screen bg-soft-white text-deep-navy-900">
  <!-- Header -->
  <header class="bg-soft-white border-b border-sage-200 sticky top-0 z-40">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      <div class="flex justify-between items-center">
        ...
        <nav class="hidden md:flex items-center space-x-6">
          <a href="/give-support" data-sveltekit-preload-data="hover" class="text-deep-navy-700 hover:text-deep-navy-900 transition-colors duration-200 font-medium">
            Give Support
          </a>
          <a href="/get-aid" data-sveltekit-preload-data="hover" class="text-deep-navy-700 hover:text-deep-navy-900 transition-colors duration-200 font-medium">
            Get Financial Aid
          </a>
          <a href="/resources/colorado" data-sveltekit-preload-data="hover" class="text-deep-navy-700 hover:text-deep-navy-900 transition-colors duration-200 font-medium">
            Resources
          </a>
        </nav>
      </div>
  ...
  {:else if error}
    ...
    <div class="max-w-2xl mx-auto px-4 py-16 text-center">
      <svg class="mx-auto h-12 w-12 text-navy text-opacity-40 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.29-1.01-5.625-2.58a1 1 0 111.25-1.562A6.962 6.962 0 0012 13.5c1.875 0 3.57-.76 4.875-2.02a1 1 0 011.25 1.562A7.962 7.962 0 0112 15z" />
      </svg>
      ...
      <a href="/resources/colorado" class="btn-primary">
        Browse Resources
      </a>
    </div>
      ...
      <div class="lg:grid lg:grid-cols-4 lg:gap-8">
        ...
        <div class="{showTOC && tableOfContents.length > 0 ? 'lg:col-span-3' : 'lg:col-span-4'}">
          ...
          <header class="mb-8">
            ...
            <!-- Meta Information -->
            <div class="flex flex-wrap items-center gap-4 text-sm text-deep-navy-700 border-b border-sage-200 pb-4">
          ...
          <!-- Content -->
          {#if pageData.content}
            <article class="prose prose-lg max-w-none prose-headings:font-serif prose-headings:text-deep-navy-900 prose-p:text-deep-navy-900 prose-p:leading-relaxed prose-a:text-deep-navy-700 prose-a:no-underline hover:prose-a:underline prose-strong:text-deep-navy-900 prose-blockquote:border-sage-400 prose-blockquote:text-deep-navy-900 prose-li:text-deep-navy-900">
              ...
              {#each pageData.content as block}
                ...
                <figure class="my-8">
                  <img
                    src={block.asset.url}
                    srcset={buildSanitySrcSet(block.asset.url, [480, 768, 1024])}
                    sizes={defaultSizes}
                    alt={block.alt || ''}
                    loading="lazy"
                    decoding="async"
                    class="w-full rounded-lg shadow-soft"
                  />
                  {#if block.caption}
            ...
            <div class="mt-12 pt-8 border-t border-navy border-opacity-10">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {#each pageData.relatedClusterPages as cluster}
                  <article class="bg-soft-white rounded-lg border border-sage-200 p-6">
                    {#if cluster.featuredImage}
                      <img
                        src={cluster.featuredImage.asset.url}
                        srcset={buildSanitySrcSet(cluster.featuredImage.asset.url, [320, 480, 768])}
                    ...
                    <h3 class="text-lg font-medium text-navy mb-2">
                      <a href="/resources/{cluster.slug.current}" class="hover:text-deep-navy-900 transition-colors">
                        {cluster.title}
                      </a>
                    </h3>
                    {#if cluster.excerpt}
          ...
          {#if relatedLinks && relatedLinks.length > 0}
            <div class="mt-12">
              <h2 class="text-2xl font-serif font-medium text-navy mb-4">Explore More</h2>
              <ul class="list-disc list-inside space-y-2">
                {#each relatedLinks as link}
                  <li><a href={link.href} data-sveltekit-preload-data="hover" class="text-deep-navy-700 hover:text-deep-navy-900">{link.title}</a></li>
                {/each}
              </ul>
            </div>
          {/if}
