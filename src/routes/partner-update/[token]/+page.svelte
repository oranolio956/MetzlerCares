<script lang="ts">
  ...
  async function verifyTokenAndLoadData() {
    try {
      ...
      error =
        err.message ||
        'This link has expired or is invalid. Please contact Metzler Foundations to log this outcome manually.'
    } finally {
      loading = false
    }
  }
  ...
  async function submitOutcome() {
    if (!selectedOutcome || !outcomeData?.outcome_id || !token) {
    ...
    try {
      ...
      const { data, error: submitError } = await supabase.functions.invoke('submit-outcome-update', {
        body: { token, outcome_metric: selectedOutcome },
        headers: { 'Content-Type': 'application/json' }
      })
      ...
      if (data?.success) {
        alert('Thank you. Your update has been logged.')
        goto('/partner-update/success') // Redirect to a generic success page
      } else {
    ...
    {:else if outcomeData}
      ...
      <div class="bg-white rounded-lg shadow-sm border border-navy border-opacity-10 p-6">
        ...
        <form on:submit|preventDefault={submitOutcome} class="space-y-6">
          <div>
            <label for="outcome" class="block text-sm font-medium text-navy mb-2"> Current Status * </label>
            <select
              id="outcome"
      ...
      <div class="text-center mt-8 text-sm text-navy text-opacity-50">
        <p>
          This secure link will expire in 7 days. Your submission is confidential and helps us track program
          effectiveness.
        </p>
      </div>
    {/if}
