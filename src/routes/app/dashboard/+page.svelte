<script lang="ts">
  ...
  function getConsentStatusColor(status: string) {
    switch (status) {
      case 'active': return 'bg-sage-100 text-sage-800';
  ...
  function getApplicationStatusColor(status: string) {
    switch (status) {
      case 'approved': return 'bg-sage-100 text-sage-800';
              ...
              {#each consents as consent}
                <div class="border border-gray-200 rounded-md p-4">
                  <div class="flex items-center justify-between mb-2">
                    ...
                    <span class={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getConsentStatusColor(consent.status || (consent.granted ? 'granted' : 'revoked'))}`}>
                ...
                <div class="border border-gray-200 rounded-md p-4">
                  <div class="flex items-center justify-between mb-3">
                    <h3 class="text-lg font-medium text-gray-900">
                      {(application.partner as any)?.facility_name || (application.partner as any)?.organization_name || 'Unknown Facility'}
                    </h3>
                    <span class={`inline-flex px-3 py-1 text-sm font-semibold rounded-full ${getApplicationStatusColor(application.status)}`}>
