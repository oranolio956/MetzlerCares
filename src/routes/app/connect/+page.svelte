<script lang="ts">
  ...
  const messages = [
    {
      sender: 'coach',
      text: 'Hey Alex! Just checking in. How are you feeling about the meeting tonight?',
      time: '2:15 PM'
    },
...
<div class="min-h-screen bg-gray-100 flex items-center justify-center p-4 font-sans">
  <!-- Mobile Device Frame -->
  <div
    class="w-full max-w-md bg-white h-[800px] rounded-[3rem] shadow-2xl border-[8px] border-gray-900 relative overflow-hidden flex flex-col"
  >
    <!-- Notch/Status Bar -->
    <div class="h-8 bg-gray-900 w-full absolute top-0 left-0 z-50 flex justify-center">
      <div class="w-32 h-6 bg-black rounded-b-xl" />
    </div>
    <div
      class="h-12 bg-white w-full flex justify-between items-end px-6 pb-2 text-xs font-bold text-gray-900 z-40 pt-8"
    >
      <span>9:41</span>
      <div class="flex gap-1">
        ...
        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"
          ><path
            fill-rule="evenodd"
            d="M17.778 8.232c-2.403-1.302-4.11-1.304-6.626.127-.655.372-1.745.496-2.365.536C5.267 9.064 2.5 10.861 2.5 12.48c0 1.333 4.057 3.372 6.398 3.372 1.495 0 4.258-2.005 4.258-4.891 0-.856-.319-1.211-.604-1.429-.99-.753-1.42-1.054-2.638-1.3H17.778zM14 14a2 2 0 11-4 0 2 2 0 014 0z"
            clip-rule="evenodd"
          /></svg
        >
        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"
    ...
    <div class="flex-1 overflow-y-auto bg-gray-50 pb-20 relative">
      ...
      <div class="bg-tech-primary p-6 pb-12 rounded-b-[2rem] shadow-lg relative z-10">
        <div class="flex justify-between items-center mb-6">
          ...
          <div class="w-10 h-10 rounded-full bg-gray-700 border-2 border-tech-accent overflow-hidden">
            <div class="w-full h-full bg-gradient-to-br from-gray-600 to-gray-800" />
          </div>
        </div>

        <!-- Streak Counter -->
        <div
          class="bg-white/10 backdrop-blur-md rounded-xl p-4 flex items-center justify-between border border-white/10"
        >
      ...
      <div class="px-6 -mt-6 relative z-20">
        <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
          ...
          <div class="space-y-3">
            {#each goals as goal}
              <div
                class="flex items-center gap-3 p-3 rounded-xl transition-colors {goal.completed
                  ? 'bg-green-50'
                  : 'bg-gray-50'}"
              >
                <div
                ...
                <div class="flex-1">
                  <div class={`font-medium text-sm ${goal.completed ? 'text-gray-500 line-through' : 'text-gray-800'}`}>
                    {goal.title}
                  </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
          <button
            class="bg-white p-4 rounded-2xl shadow-sm flex flex-col items-center gap-2 hover:bg-gray-50 transition-colors"
          >
      ...
      {#if showChat}
        <div
          class="absolute bottom-0 left-0 w-full h-3/4 bg-white rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] z-30 flex flex-col"
          transition:slide
        >
          <div class="p-4 border-b border-gray-100 flex justify-between items-center">
            <div class="flex items-center gap-3">
              <div
                class="w-10 h-10 rounded-full bg-tech-primary flex items-center justify-center text-white font-bold text-xs"
              >
                COACH
              </div>
              <div>
                ...
                <div class="text-xs text-green-500 flex items-center gap-1">
                  <span class="w-2 h-2 bg-green-500 rounded-full" /> Online
                </div>
              </div>
            </div>
          ...
          <div class="flex-1 p-4 space-y-4 overflow-y-auto bg-gray-50">
            {#each messages as msg}
              <div class={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                >
                  {msg.text}
                  <div class={`text-[10px] mt-1 ${msg.sender === 'user' ? 'text-blue-200' : 'text-gray-400'}`}>
                    {msg.time}
                  </div>
                </div>
              </div>
          ...
          <div class="p-4 border-t border-gray-100 bg-white">
            <div class="flex gap-2">
              <input
                ...
                class="flex-1 bg-gray-100 border-0 rounded-full px-4 py-2 text-sm focus:ring-2 focus:ring-tech-accent"
              />
              <button
                class="w-10 h-10 bg-tech-accent rounded-full flex items-center justify-center text-white hover:bg-blue-600 transition-colors"
              >
    ...
    <!-- Bottom Navigation -->
    <div
      class="h-20 bg-white border-t border-gray-200 flex justify-around items-center px-6 pb-4 absolute bottom-0 w-full z-20"
    >
      ...
      <button
        type="button"
        class="w-14 h-14 bg-tech-primary rounded-full -mt-8 border-4 border-gray-100 flex items-center justify-center shadow-lg hover:bg-gray-800 transition-colors focus:outline-none focus:ring-2 focus:ring-tech-accent focus:ring-offset-2"
    ...
    <!-- Home Indicator -->
    <div class="h-1 w-32 bg-black rounded-full absolute bottom-2 left-1/2 -translate-x-1/2 z-50" />
  </div>
</div>
