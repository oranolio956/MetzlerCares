<script lang="ts">
  ...
  $: if (confirmOpen) {
    setTimeout(() => {
      const el = document.getElementById('confirm-proceed') as HTMLButtonElement | null;
      el?.focus();
    }, 0);
  }
  ...
  async function disburseFunds() {
    ...
    try {
      disbursing = true;
      const res = await fetch('?/disburse', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-csrf-token': csrfToken },
        body: JSON.stringify({ application_id: application.id, amount: application.amount_requested })
      });
      if (!res.ok) {
        const j = await res.json().catch(() => ({}));
        toast(`Payment failed: ${j?.error?.message || res.statusText}`, 'error');
        return;
      }
  ...
  async function handleFileUpload(event: Event) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        if (!res.ok) {
          const j = await res.json().catch(() => ({}));
          toast(`Failed to upload ${file.name}: ${j?.error?.message || res.statusText}`, 'error');
        } else {
          const j = await res.json();
          const f = j.files?.[0];
          if (f) {
            uploadedFiles = [...uploadedFiles, { name: file.name, path: f.path, uploadedAt: f.uploadedAt, type: 'file' }];
          }
        }
      }
  ...
  async function manuallyApprove() {
    if (!application || uploadedFiles.length === 0) {
      toast('Please upload verification documents before approving', 'error');
      return;
    }

    try {
      const fd = new FormData();
      fd.append('notes', `Approved with ${uploadedFiles.length} verification documents`);
      fd.append('csrf_token', csrfToken);
      const res = await fetch('?/approve', { method: 'POST', body: fd });
      if (!res.ok) {
        toast('Failed to approve application', 'error');
      } else {
        await reloadPage();
        toast('Application manually approved', 'success');
      }
  ...
  async function denyApplication() {
    try {
      const res = await fetch('?/deny', { method: 'POST', headers: { 'x-csrf-token': csrfToken } });
      if (!res.ok) {
        toast('Failed to deny application', 'error');
      } else {
        await reloadPage();
        toast('Application denied', 'success');
      }
    } catch (err) {
      toast('An error occurred during denial', 'error');
    }
  }
...
<div class="min-h-screen bg-cream text-navy">
                ...
                {#each consents as consent}
                  <div class="border border-navy border-opacity-10 rounded-md p-4">
                    <div class="flex items-center justify-between mb-2">
                      ...
                      <span class={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getConsentStatusColor((consent.status as string) || 'unknown')}`}>
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-lg shadow-sm border border-navy border-opacity-10 p-6">
            ...
            <div class="space-y-4">
              <!-- Verification Status -->
              <div>
                <div class="block text-sm font-medium text-navy text-opacity-70 mb-1">
                  Verification Status
                </div>
              ...
              {#if manualReviewMode}
                <div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
                  ...
                  <div class="text-sm text-red-700 mb-4">
                    <p class="mb-2">
                      The automated verification check failed. The applicant's data could not be matched with federal or state databases.
                      Manual verification is required before funds can be disbursed.
                    </p>
                    <p>
                      To proceed, please contact the applicant or their referring case manager. Ask them to provide one of the following documents,
                      which you can upload below:
                    </p>
                  ...
                  <div class="flex space-x-3">
                    <button
                      on:click={() => openConfirm('Approve application with uploaded verification?', manuallyApprove)}
                    ...
                    <button
                      on:click={() => openConfirm('Deny this application?', denyApplication)}
              ...
              <div class="pt-4 border-t border-navy border-opacity-10">
                {#if application.status === 'approved' && !manualReviewMode}
                  <button
                    on:click={() => openConfirm('Confirm disbursement of funds?', disburseFunds)}
                    disabled={disbursing}
                  ...
                  >
                    {#if disbursing}
                      <svg class="animate-spin -ml-1 mr-3 h-4 w-4 text-cream inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
  ...
  {#if confirmOpen}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40" aria-modal="true" role="dialog">
      <div class="bg-white rounded-md shadow-lg w-full max-w-sm p-6">
        <h3 class="text-lg font-medium text-navy mb-3">Please Confirm</h3>
        <p class="text-navy text-opacity-80 mb-6">{confirmMessage}</p>
        <div class="flex justify-end gap-3">
          <button class="px-4 py-2 rounded-md border border-navy border-opacity-20" on:click={closeConfirm}>Cancel</button>
          <button id="confirm-proceed" class="px-4 py-2 rounded-md bg-olive text-cream" on:click={confirmProceed}>Proceed</button>
        </div>
      </div>
    </div>
  {/if}
<svelte:window on:keydown={handleConfirmKey} />
