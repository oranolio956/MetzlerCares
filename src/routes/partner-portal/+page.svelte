<script lang="ts">
  ...
  function getStatusColor(status: string): string {
    switch (status) {
      ...
      case 'discharged': return 'text-blue-600 bg-blue-50';
      case 'transferred': return 'text-orange-600 bg-orange-50';
            ...
            <div class="space-y-3">
              {#each clients as client}
                <button
                  ...
                  class="w-full text-left p-4 rounded-xl border-2 transition-all duration-200 {selectedClient?.id === client.id ? 'border-primary bg-primary bg-opacity-5' : 'border-gray-100 hover:border-primary hover:bg-gray-50'}"
                >
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
              <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex gap-2">
                  ...
                  <span class="px-3 py-1 rounded-full text-sm font-medium {getConsentStatusColor(selectedClient.consent_status)}">
              ...
              <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  <label for="discharge-date" class="block text-sm font-medium text-gray-medium mb-1">Discharge Date</label>
                  <div id="discharge-date" class="text-gray-dark">{formatDate(selectedClient.discharge_date) || 'Not set'}</div>
                </div>
                <div>
                  <label for="consent-signed" class="block text-sm font-medium text-gray-medium mb-1">Consent Signed</label>
                  <div id="consent-signed" class="text-gray-dark">{formatDate(selectedClient.consent_signed_at) || 'Not signed'}</div>
                </div>
              </div>
            </div>
              ...
              <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div class="text-center p-4 rounded-xl bg-gray-50 border-2 border-gray-200">
                    <div class="text-2xl font-bold text-gray-600">{clientCards.filter(c => c.status === 'todo').length}</div>
                    <div class="text-sm text-gray-medium">To Do</div>
                  </div>
                  <div class="text-center p-4 rounded-xl bg-yellow-50 border-2 border-yellow-200">
                    <div class="text-2xl font-bold text-yellow-600">{clientCards.filter(c => c.status === 'in_progress').length}</div>
                  ...
                  <div class="text-center p-4 rounded-xl bg-blue-50 border-2 border-blue-200">
                    <div class="text-2xl font-bold text-blue-600">{clientCards.filter(c => c.status === 'submitted').length}</div>
                  ...
                  <div class="text-center p-4 rounded-xl bg-green-50 border-2 border-green-200">
                    <div class="text-2xl font-bold text-green-600">{clientCards.filter(c => c.status === 'complete').length}</div>
              ...
              <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="space-y-3">
                  {#each clientCards as card}
                    <div class="flex items-center justify-between p-4 rounded-xl border-2 {getCardStatusColor(card.status)}">
