...
export async function POST({ request, cookies }) {
  try {
    ...
    for (const cityName of cities) {
      const location = COLORADO_LOCATIONS.find(loc => 
      ...
      try {
        ...
        const internalLinks = coloradoInternalLinking.generateInternalLinks(
          content.canonical, 
          content.content
        );
...
export async function DELETE({ request, cookies }) {
  try {
    ...
    if (action === 'optimize') {
      ...
      const highPriorityCities = COLORADO_LOCATIONS
