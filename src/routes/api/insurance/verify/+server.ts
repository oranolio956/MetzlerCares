...
const insuranceProviders = {
  ...
  'Humana': { verificationTime: 2200, successRate: 0.90 },
  'Kaiser Permanente': { verificationTime: 1800, successRate: 0.92 },
  'UnitedHealthcare': { verificationTime: 3200, successRate: 0.80 },
  'TriCare': { verificationTime: 2400, successRate: 0.94 },
  'Other': { verificationTime: 4000, successRate: 0.70 }
};
  ...
  try {
    ...
    if (cachedResult) {
      ...
      return json({
        ...
        cached: true,
        message: cachedResult.verification_status === 'verified' ? 'Insurance verified successfully' : 'Insurance verification failed'
      });
    }
    
    // Simulate API verification delay
    const providerConfig = insuranceProviders[provider as keyof typeof insuranceProviders] || insuranceProviders['Other'];
    ...
    if (supabase) await supabase.from('insurance_verification_cache').insert({
