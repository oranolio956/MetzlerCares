<script lang="ts">
  ...
  async function requestDataDeletion() {
    if (
      !confirm(
        'Are you sure you want to request data deletion? This action cannot be undone and may affect your ability to receive services.'
      )
    ) {
      return
    }
    ...
    try {
      ...
      alert(
        'Your data deletion request has been submitted. A team member will review your request and contact you within 30 days.'
      )
    } catch (error) {
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Manage your privacy settings and exercise your data rights under the Colorado Privacy Act."
  />
</svelte:head>
  ...
  <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-navy border-opacity-10 p-6 mb-8">
      ...
      {:else if consents.length === 0}
        <div class="text-center py-8">
          <svg
            class="mx-auto h-12 w-12 text-navy text-opacity-40 mb-3"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
    ...
    <div class="bg-white rounded-lg shadow-sm border border-navy border-opacity-10 p-6">
      ...
      <div class="prose prose-sm max-w-none text-navy text-opacity-80 mb-8">
        <p>
          Under the Colorado Privacy Act, you have the right to request a copy of your data or request the deletion of
          your personal data. Please note that for legal and medical compliance, we are required to retain some records
          for a set period, but we will delete all non-essential data upon your request.
        </p>
        <p>
          All requests will be reviewed by our compliance team and processed within 30 days. You will receive email
          confirmation of your request and its completion.
        </p>
      </div>
